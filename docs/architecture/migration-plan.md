...
|---|---|---|---|---|
| 1 | cross-cutting helpers | `src/platform` + Foundation public API | business modules depend on Foundation | Low; revert import-only commit |
| 2 | `services/catalog-item-repository.ts` and catalog routes | Catalog module | routes depend on Catalog query port via `src/modules/catalog` | Medium; compatibility adapter retained |
...
| 4 | Opportunity/Company reads | Sales module | Proposal routes use Sales ports via `src/modules/sales` | Low; Twenty repository remains adapter |
